...
def setup_warcraft_logs(bot):
    ...
    async def on_started(_: hikari.StartedEvent) -> None:
        ...
        for name, source in config['log_sources'].items():
            ...
            schedule_log_check(name, formatted_url, f"logs/{source['filename']}", color_int, channels)

    def schedule_log_check(job_id, url, filename, color, channels):
        bot.d.sched.add_job(check_and_announce_logs, CronTrigger(minute="*/2"),
                            args=[url, filename, color, job_id, channels],
                            misfire_grace_time=None, id=job_id)

    async def check_and_announce_logs(url, filename, color, log_source_name, channels):
        try:
            ...
            if logs_id != previous_logs_id:
                await announce_new_logs(first_log, logs_id, filename, color, log_source_name, channels)
            else:
    ...
    async def announce_new_logs(log, logs_id, filename, color, log_source_name, channels):
        title = log['title']
        owner = log['owner']
        starttime = log['start']
